...
public:
    ...
    static void LineSearch(Foo& f, const LBFGSParam<Scalar>& param, const Vector& xp, const Vector& drt, const Scalar& step_max, Scalar& step,
                           Scalar& fx, Vector& grad, Scalar& dg, Vector& x) {
        ...
        if (dg_init > 0) throw std::logic_error("the moving direction increases the objective function value");
        ...
        for (iter = 0; iter < param.max_linesearch; iter++) {
            ...
            if (fx > fx_init + step * test_decr || (fx != fx)) {
                ...
                if (param.linesearch == LBFGS_LINESEARCH_BACKTRACKING_ARMIJO) break;
                ...
                if (dg < param.wolfe * dg_init) {
                    ...
                    if (param.linesearch == LBFGS_LINESEARCH_BACKTRACKING_WOLFE) break;
            ...
            if (step < param.min_step) throw std::runtime_error("the line search step became smaller than the minimum value allowed");
        ...
        if (iter >= param.max_linesearch) throw std::runtime_error("the line search routine reached the maximum number of iterations");
    }
};
